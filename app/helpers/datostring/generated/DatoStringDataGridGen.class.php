<?php
	 ...
	 *
	 */
	class DatoStringDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for dato_string's properties, or you
			// can traverse down QQN::dato_string() to display fields that are down the hierarchy)
			$this->MetaAddColumn('IdDatoString')->Name = QApplication::Translate('IdDatoString');
			$this->MetaAddColumn(QQN::DatoString()->IdCampoObject)->Name = QApplication::Translate('IdCampoObject');
			$this->MetaAddColumn(QQN::DatoString()->IdPersonalObject)->Name = QApplication::Translate('IdPersonalObject');
         ...
         * way of adding a column to this DatoString datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the DatoString object, or it can be any QQNode extending from QQN::DatoString().
         * 
         ...
         * sorting using two fields from the DatoString object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DatoString
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdDatoString=<?=urlencode($_ITEM->IdDatoString)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdDatoString)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdDatoString";
			$strHtml = '<a href="datostring/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDatoString, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(DatoString $objDatoString) {
                        $strControlId = "";
                        $strControlId .= "x" . $objDatoString->IdDatoString;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This DatoString');
					...
					if (isset($this->ParentControl->pnlEditDatoString)){
		                if ($this->ParentControl->pnlEditDatoString->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof DatoStringIndexPanel){
                             $this->ParentControl->pnlEditDatoString = new DatoStringEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditDatoString->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/datostring/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = DatoString::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from DatoString, given the clauses above
            $this->DataSource = DatoString::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DatoString-based QQNode.
		 * It will also verify that it is a proper DatoString-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'dato_string') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "dato_string".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdDatoString': return QQN::DatoString()->IdDatoString;
				case 'IdCampo': return QQN::DatoString()->IdCampo;
				case 'IdCampoObject': return QQN::DatoString()->IdCampoObject;
				case 'IdPersonal': return QQN::DatoString()->IdPersonal;
				case 'IdPersonalObject': return QQN::DatoString()->IdPersonalObject;
				case 'IdDesignacion': return QQN::DatoString()->IdDesignacion;
				case 'Valor': return QQN::DatoString()->Valor;
				case 'FechaModificacion': return QQN::DatoString()->FechaModificacion;
				default: throw new QCallerException('Simple Property not found in DatoStringDataGrid content: ' . $mixContent);
