<?php
	 ...
	 * listing a collection of Designacion objects.  It includes
	 ...
	 *
	 */
	class DesignacionDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for designacion's properties, or you
			// can traverse down QQN::designacion() to display fields that are down the hierarchy)
			...
			$this->MetaAddColumn(QQN::Designacion()->IdUnidadServicioObject)->Name = QApplication::Translate('IdUnidadServicioObject');
		}


		/**
         * Given the description of the Column's contents, this is a simple, express
         * way of adding a column to this Designacion datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the Designacion object, or it can be any QQNode extending from QQN::Designacion().
         * 
         ...
         * sorting using two fields from the Designacion object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Designacion
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdDesignacion=<?=urlencode($_ITEM->IdDesignacion)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdDesignacion)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdDesignacion";
			$strHtml = '<a href="designacion/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDesignacion, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Designacion $objDesignacion) {
                        $strControlId = "";
                        $strControlId .= "x" . $objDesignacion->IdDesignacion;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This Designacion');
					...
					if (isset($this->ParentControl->pnlEditDesignacion)){
		                if ($this->ParentControl->pnlEditDesignacion->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof DesignacionIndexPanel){
                             $this->ParentControl->pnlEditDesignacion = new DesignacionEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditDesignacion->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/designacion/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Designacion::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Designacion, given the clauses above
            $this->DataSource = Designacion::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Designacion-based QQNode.
		 * It will also verify that it is a proper Designacion-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'designacion') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "designacion".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdDesignacion': return QQN::Designacion()->IdDesignacion;
				case 'Nombre': return QQN::Designacion()->Nombre;
				case 'Descripcion': return QQN::Designacion()->Descripcion;
				case 'IdUnidadServicio': return QQN::Designacion()->IdUnidadServicio;
				case 'IdUnidadServicioObject': return QQN::Designacion()->IdUnidadServicioObject;
				default: throw new QCallerException('Simple Property not found in DesignacionDataGrid content: ' . $mixContent);
