<?php
	 ...
	 * listing a collection of Cuadro objects.  It includes
	 ...
	 *
	 */
	class CuadroDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for cuadro's properties, or you
			// can traverse down QQN::cuadro() to display fields that are down the hierarchy)
			$this->MetaAddColumn('IdCuadro')->Name = QApplication::Translate('IdCuadro');
			$this->MetaAddColumn('Pagina')->Name = QApplication::Translate('Pagina');
			$this->MetaAddColumn('Orden')->Name = QApplication::Translate('Orden');
			$this->MetaAddColumn('Nombre')->Name = QApplication::Translate('Nombre');
			$this->MetaAddColumn('Numero')->Name = QApplication::Translate('Numero');
			$this->MetaAddColumn('Error')->Name = QApplication::Translate('Error');
		}


		/**
         * Given the description of the Column's contents, this is a simple, express
         * way of adding a column to this Cuadro datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the Cuadro object, or it can be any QQNode extending from QQN::Cuadro().
         * 
         ...
         * sorting using two fields from the Cuadro object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Cuadro
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdCuadro=<?=urlencode($_ITEM->IdCuadro)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdCuadro)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdCuadro";
			$strHtml = '<a href="cuadro/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdCuadro, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Cuadro $objCuadro) {
                        $strControlId = "";
                        $strControlId .= "x" . $objCuadro->IdCuadro;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This Cuadro');
					...
					if (isset($this->ParentControl->pnlEditCuadro)){
		                if ($this->ParentControl->pnlEditCuadro->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof CuadroIndexPanel){
                             $this->ParentControl->pnlEditCuadro = new CuadroEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditCuadro->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/cuadro/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Cuadro::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Cuadro, given the clauses above
            $this->DataSource = Cuadro::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Cuadro-based QQNode.
		 * It will also verify that it is a proper Cuadro-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'cuadro') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "cuadro".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdCuadro': return QQN::Cuadro()->IdCuadro;
				case 'Pagina': return QQN::Cuadro()->Pagina;
				case 'Orden': return QQN::Cuadro()->Orden;
				case 'Nombre': return QQN::Cuadro()->Nombre;
				case 'Numero': return QQN::Cuadro()->Numero;
				case 'Error': return QQN::Cuadro()->Error;
				default: throw new QCallerException('Simple Property not found in CuadroDataGrid content: ' . $mixContent);
