<?php
	 ...
	 *
	 */
	class DatoDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for dato's properties, or you
			// can traverse down QQN::dato() to display fields that are down the hierarchy)
			$this->MetaAddColumn('IdDato')->Name = QApplication::Translate('IdDato');
			$this->MetaAddColumn(QQN::Dato()->IdCampoObject)->Name = QApplication::Translate('IdCampoObject');
			$this->MetaAddColumn(QQN::Dato()->IdPersonalObject)->Name = QApplication::Translate('IdPersonalObject');
			$this->MetaAddColumn(QQN::Dato()->IdDesignacionObject)->Name = QApplication::Translate('IdDesignacionObject');
         ...
         * way of adding a column to this Dato datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the Dato object, or it can be any QQNode extending from QQN::Dato().
         * 
         ...
         * sorting using two fields from the Dato object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Dato
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdDato=<?=urlencode($_ITEM->IdDato)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdDato)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdDato";
			$strHtml = '<a href="dato/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDato, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Dato $objDato) {
                        $strControlId = "";
                        $strControlId .= "x" . $objDato->IdDato;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This Dato');
					...
					if (isset($this->ParentControl->pnlEditDato)){
		                if ($this->ParentControl->pnlEditDato->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof DatoIndexPanel){
                             $this->ParentControl->pnlEditDato = new DatoEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditDato->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/dato/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Dato::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Dato, given the clauses above
            $this->DataSource = Dato::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Dato-based QQNode.
		 * It will also verify that it is a proper Dato-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'dato') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "dato".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdDato': return QQN::Dato()->IdDato;
				case 'IdCampo': return QQN::Dato()->IdCampo;
				case 'IdCampoObject': return QQN::Dato()->IdCampoObject;
				case 'IdPersonal': return QQN::Dato()->IdPersonal;
				case 'IdPersonalObject': return QQN::Dato()->IdPersonalObject;
				case 'IdDesignacion': return QQN::Dato()->IdDesignacion;
				case 'IdDesignacionObject': return QQN::Dato()->IdDesignacionObject;
				case 'Valor': return QQN::Dato()->Valor;
				case 'FechaModificacion': return QQN::Dato()->FechaModificacion;
				default: throw new QCallerException('Simple Property not found in DatoDataGrid content: ' . $mixContent);
