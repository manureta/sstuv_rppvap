 ...
 * listing a collection of AprobacionGeodesia objects.  It includes
 ...
 *
 */
class AprobacionGeodesiaDataGridGen extends QFilteredDataGrid {
    ...
    public function __construct($objParentObject, $strColumnsArray = null, $strControlId = null) {
        ...
        $this->UseAjax = true;
        $this->strEntidad = 'AprobacionGeodesia';
        $this->strNoun = AprobacionGeodesia::Noun();
        $this->strNounPlural = AprobacionGeodesia::NounPlural();
...
/**
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdAprobacionGeodesia";
			$strHtml = '<a href="aprobaciongeodesia/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Id, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(AprobacionGeodesia $objAprobacionGeodesia) {
                        // se concatenan todos los campos PK
                        $strControlId = "";
                        $strControlId .= "x" . $objAprobacionGeodesia->Id;
					...
					if (isset($this->ParentControl->pnlEditAprobacionGeodesia)){
		                if ($this->ParentControl->pnlEditAprobacionGeodesia->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof AprobacionGeodesiaIndexPanelGen){
                             $this->ParentControl->pnlEditAprobacionGeodesia = new AprobacionGeodesiaEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditAprobacionGeodesia->Visible=true;
                             $this->ParentControl->dtgAprobacionGeodesias->Visible=false;
                             $this->ParentControl->btnCreateNew->Visible=false;
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/aprobaciongeodesia/edit/".$strParameter);
                          }
                 }
...
/**
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = AprobacionGeodesia::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from AprobacionGeodesia, given the clauses above
            $this->DataSource = AprobacionGeodesia::QueryArray($this->Conditions, $objClauses);
        }

/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a AprobacionGeodesia-based QQNode.
		 * It will also verify that it is a proper AprobacionGeodesia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'aprobacion_geodesia') {
    ...
    public static function SetValor($strColumnName, $mixValue) {
        AprobacionGeodesiaDataGrid::$strColumnsArray[$strColumnName] = false;
        AprobacionGeodesiaDataGrid::$objStaticConditionsArray[$strColumnName] = QQ::Equal(QQN::AprobacionGeodesia()->$strColumnName, $mixValue);
    }
