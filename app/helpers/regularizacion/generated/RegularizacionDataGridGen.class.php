 ...
 * listing a collection of Regularizacion objects.  It includes
 ...
 *
 */
class RegularizacionDataGridGen extends QFilteredDataGrid {
    ...
    public function __construct($objParentObject, $strColumnsArray = null, $strControlId = null) {
        ...
        $this->UseAjax = true;
        $this->strEntidad = 'Regularizacion';
        $this->strNoun = Regularizacion::Noun();
        $this->strNounPlural = Regularizacion::NounPlural();
    ...
    protected function addAllColumns() {
        ...
        // Create the Columns (note that you can use strings for regularizacion's properties, or you
        // can traverse down QQN::regularizacion() to display fields that are down the hierarchy)
...
/**
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdRegularizacion";
			$strHtml = '<a href="regularizacion/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Id, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Regularizacion $objRegularizacion) {
                        // se concatenan todos los campos PK
                        $strControlId = "";
                        $strControlId .= "x" . $objRegularizacion->Id;
					...
					if (isset($this->ParentControl->pnlEditRegularizacion)){
		                if ($this->ParentControl->pnlEditRegularizacion->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof RegularizacionIndexPanelGen){
                             $this->ParentControl->pnlEditRegularizacion = new RegularizacionEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditRegularizacion->Visible=true;
                             $this->ParentControl->dtgRegularizaciones->Visible=false;
                             $this->ParentControl->btnCreateNew->Visible=false;
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/regularizacion/edit/".$strParameter);
                          }
                 }
...
/**
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Regularizacion::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Regularizacion, given the clauses above
            $this->DataSource = Regularizacion::QueryArray($this->Conditions, $objClauses);
        }

/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Regularizacion-based QQNode.
		 * It will also verify that it is a proper Regularizacion-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'regularizacion') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "regularizacion".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Regularizacion()->Id;
    ...
    public static function SetValor($strColumnName, $mixValue) {
        RegularizacionDataGrid::$strColumnsArray[$strColumnName] = false;
        RegularizacionDataGrid::$objStaticConditionsArray[$strColumnName] = QQ::Equal(QQN::Regularizacion()->$strColumnName, $mixValue);
    }
