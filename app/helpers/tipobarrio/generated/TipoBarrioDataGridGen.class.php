 ...
 * listing a collection of TipoBarrio objects.  It includes
 ...
 *
 */
class TipoBarrioDataGridGen extends QFilteredDataGrid {
    ...
    public function __construct($objParentObject, $strColumnsArray = null, $strControlId = null) {
        ...
        $this->UseAjax = true;
        $this->strEntidad = 'TipoBarrio';
        $this->strNoun = TipoBarrio::Noun();
        $this->strNounPlural = TipoBarrio::NounPlural();
    ...
    protected function addAllColumns() {
        ...
        // Create the Columns (note that you can use strings for tipo_barrio's properties, or you
        // can traverse down QQN::tipo_barrio() to display fields that are down the hierarchy)
        if (TipoBarrioDataGrid::$strColumnsArray['Id']) $this->MetaAddColumn('Id')->Title = QApplication::Translate('Id');
        if (TipoBarrioDataGrid::$strColumnsArray['Tipo']) $this->MetaAddColumn('Tipo')->Title = QApplication::Translate('Tipo');
    }
...
/**
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdTipoBarrio";
			$strHtml = '<a href="tipobarrio/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->Id, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(TipoBarrio $objTipoBarrio) {
                        // se concatenan todos los campos PK
                        $strControlId = "";
                        $strControlId .= "x" . $objTipoBarrio->Id;
					...
					if (isset($this->ParentControl->pnlEditTipoBarrio)){
		                if ($this->ParentControl->pnlEditTipoBarrio->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof TipoBarrioIndexPanelGen){
                             $this->ParentControl->pnlEditTipoBarrio = new TipoBarrioEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditTipoBarrio->Visible=true;
                             $this->ParentControl->dtgTipoBarrios->Visible=false;
                             $this->ParentControl->btnCreateNew->Visible=false;
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/tipobarrio/edit/".$strParameter);
                          }
                 }
...
/**
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = TipoBarrio::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from TipoBarrio, given the clauses above
            $this->DataSource = TipoBarrio::QueryArray($this->Conditions, $objClauses);
        }

/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a TipoBarrio-based QQNode.
		 * It will also verify that it is a proper TipoBarrio-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'tipo_barrio') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "tipo_barrio".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::TipoBarrio()->Id;
				case 'Tipo': return QQN::TipoBarrio()->Tipo;
				default: throw new QCallerException('Simple Property not found in TipoBarrioDataGrid content: ' . $mixContent);
    ...
    public static function SetValor($strColumnName, $mixValue) {
        TipoBarrioDataGrid::$strColumnsArray[$strColumnName] = false;
        TipoBarrioDataGrid::$objStaticConditionsArray[$strColumnName] = QQ::Equal(QQN::TipoBarrio()->$strColumnName, $mixValue);
    }
