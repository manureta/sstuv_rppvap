<?php
	 ...
	 *
	 */
	class NoticiaDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			$this->MetaAddColumn('IdNoticia')->Name = QApplication::Translate('IdNoticia');
			...
			$this->MetaAddColumn('Arriba')->Name = QApplication::Translate('Arriba');
			$this->MetaAddColumn('Login')->Name = QApplication::Translate('Login');
		}


		/**
         * Given the description of the Column's contents, this is a simple, express
         * way of adding a column to this Noticia datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the Noticia object, or it can be any QQNode extending from QQN::Noticia().
         * 
         ...
         * sorting using two fields from the Noticia object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Noticia
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdNoticia=<?=urlencode($_ITEM->IdNoticia)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdNoticia)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdNoticia";
			$strHtml = '<a href="noticia/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdNoticia, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Noticia $objNoticia) {
                        $strControlId = "";
                        $strControlId .= "x" . $objNoticia->IdNoticia;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This Noticia');
					...
					if (isset($this->ParentControl->pnlEditNoticia)){
		                if ($this->ParentControl->pnlEditNoticia->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof NoticiaIndexPanel){
                             $this->ParentControl->pnlEditNoticia = new NoticiaEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditNoticia->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/noticia/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Noticia::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Noticia, given the clauses above
            $this->DataSource = Noticia::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Noticia-based QQNode.
		 * It will also verify that it is a proper Noticia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'noticia') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "noticia".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdNoticia': return QQN::Noticia()->IdNoticia;
				case 'Titulo': return QQN::Noticia()->Titulo;
				case 'Texto': return QQN::Noticia()->Texto;
				case 'Fecha': return QQN::Noticia()->Fecha;
				case 'Mostrar': return QQN::Noticia()->Mostrar;
				case 'Arriba': return QQN::Noticia()->Arriba;
				case 'Login': return QQN::Noticia()->Login;
				default: throw new QCallerException('Simple Property not found in NoticiaDataGrid content: ' . $mixContent);
