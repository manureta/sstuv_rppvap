<?php
	 ...
	 * listing a collection of SistemaVersion objects.  It includes
	 ...
	 *
	 */
	class SistemaVersionDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for sistema_version's properties, or you
			// can traverse down QQN::sistema_version() to display fields that are down the hierarchy)
			$this->MetaAddColumn('IdSistemaVersion')->Name = QApplication::Translate('IdSistemaVersion');
			$this->MetaAddColumn('CodigoVersion')->Name = QApplication::Translate('CodigoVersion');
			$this->MetaAddColumn('NombreVersion')->Name = QApplication::Translate('NombreVersion');
			$this->MetaAddColumn('DescripcionVersion')->Name = QApplication::Translate('DescripcionVersion');
			$this->MetaAddColumn('FilePath')->Name = QApplication::Translate('FilePath');
			$this->MetaAddColumn('Fecha')->Name = QApplication::Translate('Fecha');
			$this->MetaAddColumn('Orden')->Name = QApplication::Translate('Orden');
		}


		/**
         * Given the description of the Column's contents, this is a simple, express
         * way of adding a column to this SistemaVersion datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the SistemaVersion object, or it can be any QQNode extending from QQN::SistemaVersion().
         * 
         ...
         * sorting using two fields from the SistemaVersion object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from SistemaVersion
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdSistemaVersion=<?=urlencode($_ITEM->IdSistemaVersion)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdSistemaVersion)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdSistemaVersion";
			$strHtml = '<a href="sistemaversion/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdSistemaVersion, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(SistemaVersion $objSistemaVersion) {
                        $strControlId = "";
                        $strControlId .= "x" . $objSistemaVersion->IdSistemaVersion;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This SistemaVersion');
					...
					if (isset($this->ParentControl->pnlEditSistemaVersion)){
		                if ($this->ParentControl->pnlEditSistemaVersion->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof SistemaVersionIndexPanel){
                             $this->ParentControl->pnlEditSistemaVersion = new SistemaVersionEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditSistemaVersion->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/sistemaversion/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = SistemaVersion::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from SistemaVersion, given the clauses above
            $this->DataSource = SistemaVersion::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a SistemaVersion-based QQNode.
		 * It will also verify that it is a proper SistemaVersion-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'sistema_version') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "sistema_version".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdSistemaVersion': return QQN::SistemaVersion()->IdSistemaVersion;
				case 'CodigoVersion': return QQN::SistemaVersion()->CodigoVersion;
				case 'NombreVersion': return QQN::SistemaVersion()->NombreVersion;
				case 'DescripcionVersion': return QQN::SistemaVersion()->DescripcionVersion;
				case 'FilePath': return QQN::SistemaVersion()->FilePath;
				case 'Fecha': return QQN::SistemaVersion()->Fecha;
				case 'Orden': return QQN::SistemaVersion()->Orden;
				default: throw new QCallerException('Simple Property not found in SistemaVersionDataGrid content: ' . $mixContent);
