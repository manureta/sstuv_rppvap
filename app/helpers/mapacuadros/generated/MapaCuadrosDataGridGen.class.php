<?php
	 ...
	 *
	 */
	class MapaCuadrosDataGridGen extends QFilteredDataGrid {
		/**
		...
		protected function addColumns() {
		 // Use the MetaDataGrid functionality to add Columns for this datagrid
			...
			// Create the Other Columns (note that you can use strings for mapa_cuadros's properties, or you
			// can traverse down QQN::mapa_cuadros() to display fields that are down the hierarchy)
			$this->MetaAddColumn(QQN::MapaCuadros()->IdDefinicionCuadroObject)->Name = QApplication::Translate('IdDefinicionCuadroObject');
			$this->MetaAddColumn('COferta')->Name = QApplication::Translate('COferta');
			$this->MetaAddColumn('CEstado')->Name = QApplication::Translate('CEstado');
		}


		/**
         * Given the description of the Column's contents, this is a simple, express
         * way of adding a column to this MapaCuadros datagrid.  The description of a column's
         * content can be either a text string description of a simple field name
         * in the MapaCuadros object, or it can be any QQNode extending from QQN::MapaCuadros().
         * 
         ...
         * sorting using two fields from the MapaCuadros object.
         *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from MapaCuadros
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdDefinicionCuadro=<?=urlencode($_ITEM->IdDefinicionCuadro)?>&intCOferta=<?=urlencode($_ITEM->COferta)?>&intCEstado=<?=urlencode($_ITEM->CEstado)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdDefinicionCuadro)?>/<?=urlencode($_ITEM->COferta)?>/<?=urlencode($_ITEM->CEstado)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdMapaCuadros";
			$strHtml = '<a href="mapacuadros/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDefinicionCuadro . "," . $_ITEM->COferta . "," . $_ITEM->CEstado, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(MapaCuadros $objMapaCuadros) {
                        $strControlId = "";
                        $strControlId .= "x" . $objMapaCuadros->IdDefinicionCuadro;
                        $strControlId .= "x" . $objMapaCuadros->COferta;
                        $strControlId .= "x" . $objMapaCuadros->CEstado;
	                ...
	                if (!$btnEdit) {
	                    ...
	                    $btnEdit->Text = QApplication::Translate('Edit This MapaCuadros');
					...
					if (isset($this->ParentControl->pnlEditMapaCuadros)){
		                if ($this->ParentControl->pnlEditMapaCuadros->Visible)
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof MapaCuadrosIndexPanel){
                             $this->ParentControl->pnlEditMapaCuadros = new MapaCuadrosEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditMapaCuadros->Visible=true;
                             $this->ParentControl->btnCreateNew->Enabled=false;
                             $this->ParentControl->Refresh();
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/mapacuadros/edit/".$strParameter);
                          }
                 }
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = MapaCuadros::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from MapaCuadros, given the clauses above
            $this->DataSource = MapaCuadros::QueryArray($this->Conditions, $objClauses);
        }
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a MapaCuadros-based QQNode.
		 * It will also verify that it is a proper MapaCuadros-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'mapa_cuadros') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "mapa_cuadros".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdDefinicionCuadro': return QQN::MapaCuadros()->IdDefinicionCuadro;
				case 'IdDefinicionCuadroObject': return QQN::MapaCuadros()->IdDefinicionCuadroObject;
				case 'COferta': return QQN::MapaCuadros()->COferta;
				case 'CEstado': return QQN::MapaCuadros()->CEstado;
				default: throw new QCallerException('Simple Property not found in MapaCuadrosDataGrid content: ' . $mixContent);
