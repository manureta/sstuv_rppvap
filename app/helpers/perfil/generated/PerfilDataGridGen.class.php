 ...
 *
 */
class PerfilDataGridGen extends QFilteredDataGrid {
    ...
    public function __construct($objParentObject, $strColumnsArray = null, $strControlId = null) {
        ...
        $this->UseAjax = true;
        $this->strEntidad = 'Perfil';
        $this->strNoun = Perfil::Noun();
        $this->strNounPlural = Perfil::NounPlural();
    ...
    protected function addAllColumns() {
        ...
        // Create the Columns (note that you can use strings for perfil's properties, or you
        // can traverse down QQN::perfil() to display fields that are down the hierarchy)
        if (PerfilDataGrid::$strColumnsArray['IdPerfil']) $this->MetaAddColumn('IdPerfil')->Title = QApplication::Translate('IdPerfil');
        if (PerfilDataGrid::$strColumnsArray['Nombre']) $this->MetaAddColumn('Nombre')->Title = QApplication::Translate('Nombre');
        if (PerfilDataGrid::$strColumnsArray['Descripcion']) $this->MetaAddColumn('Descripcion')->Title = QApplication::Translate('Descripcion');
    }
...
/**
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= '?intIdPerfil=<?=urlencode($_ITEM->IdPerfil)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdPerfil)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$intId="IdPerfil";
			$strHtml = '<a href="perfil/edit/<?=$_ITEM->'.$intId.'?>" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdPerfil, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		
		public function EditColumn_Render(Perfil $objPerfil) {
		 ...
		 public function btnEdit_Click($strFormId, $strControlId, $strParameter) {
                         ...
                         if ($this->ParentControl instanceof PerfilIndexPanelGen){
                             $this->ParentControl->pnlEditPerfil = new PerfilEditPanel($this->ParentControl,null,$strParameter);
                             $this->ParentControl->pnlEditPerfil->Visible=true;
                             $this->ParentControl->dtgPerfiles->Visible=false;
                             $this->ParentControl->btnCreateNew->Visible=false;
                          }else{
                             QApplication::Redirect(__VIRTUAL_DIRECTORY__."/perfil/edit/".$strParameter);
                          }
                 }
...
/**
        ...
        public function MetaDataBinder() {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = Perfil::QueryCount($this->Conditions);
            }
            ...
            // Set the DataSource to be a Query result from Perfil, given the clauses above
            $this->DataSource = Perfil::QueryArray($this->Conditions, $objClauses);
        }

/**
		 * Used internally by the Meta-based Add Column tools.
		 *
		 * Given a QQNode or a Text String, this will return a Perfil-based QQNode.
		 * It will also verify that it is a proper Perfil-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'perfil') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "perfil".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdPerfil': return QQN::Perfil()->IdPerfil;
				case 'Nombre': return QQN::Perfil()->Nombre;
				case 'Descripcion': return QQN::Perfil()->Descripcion;
				default: throw new QCallerException('Simple Property not found in PerfilDataGrid content: ' . $mixContent);
    ...
    public static function SetValor($strColumnName, $mixValue) {
        PerfilDataGrid::$strColumnsArray[$strColumnName] = false;
        PerfilDataGrid::$objStaticConditionsArray[$strColumnName] = QQ::Equal(QQN::Perfil()->$strColumnName, $mixValue);
    }
