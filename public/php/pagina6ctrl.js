...
function procesarObjEstudio(str_row_id,has_delete,intCantMaxTablas){
    ...
    $("#estudio_line_"+str_row_id).find("[id^=b6_d261_a_1_text]").change();
}
...
function setAutoCompleteTextBox(obj,objText){
    ...
    if( strInputName.indexOf("b6_d261_a_1") == 0  ){ 
        ConfigureAutoCompleteTextBox(objText);
        objText.change(
            function(){ 
                var t = $(this).val();
                if(t=="") t="Nuevo estudios de grado en el nivel superior ";
...
function getAutoCompleteData(objIdTag){
    if( objIdTag.indexOf("b6_d261_a_1") == 0  ){ //Titulo Posgrado
        return {
            cat: "titulo_grado"
        };
    }
...
function loadSelectValues(obj,str_row_panel){
    ...
    if( strSelectName.indexOf("b6_d263_o_1") == 0  ){ 
