qcodo.confirmMessage = "ATENCIÓN: Los cambios que no se hayan verificado se perderán: ¿está seguro que quiere abandonar la carga?";
...
////////////////////////////////////////////
	...
	qcodo.postBack = function(strForm, strControl, strEvent, strParameter) {
        ...
        qcodo.redirect = function(strLocation) {
            if (window.onbeforeunload && typeof window.onbeforeunload() != 'undefined') {
                if (!confirm(window.onbeforeunload())) return;
	...
	qcodo.postAjax = function(strForm, strControl, strEvent, strParameter, strWaitIconControlId) {
        if (qcodo.EditPanel){
            if(!qc.EditPanel.confirmPostAjax(strControl))
                return;
        }            
			...
			if (strWaitIconControlId) {
				...
				if (this.objAjaxWaitIcon)
					this.objAjaxWaitIcon.style.display = 'inline';
                            ...
                            if (qcodo.objLoadingPanel) {
                                qcodo.objLoadingPanel.style.display = 'inline';
                            }
                        }
...
//								alert('AJAX Response Received');
								...
								if (!objXmlDoc) {
									if(SERVER_INSTANCE != "prod"){
										alert("Ocurrio un error durante el parseo de la respuesta AJAX.\r\n\r\nLa respuesta de error aparecera en un nuevo popup.");
										...
										// primero mando el mensaje a un php.
										var msgError = objRequest.responseText;
										var url = qc.phpAssets + "/errorAjax.php";
										var ajax = objRequest;
										...
										ajax.send('msg="'+msgError+'"');

										*/
                                                                                // ahora alerto
										alert("La ultima accion genero un error no esperado en el servidor.\r\n\r\nEs probable que no se haya efectuado la accion requerida.");
									}
								} else {
									var intLength = 0;
								...
								if(SERVER_INSTANCE != "prod"){
									alert(objExc.message + "\r\nen linea numero " + objExc.lineNumber + "\r\nen fichero " + objExc.fileName);
									alert("Ocurrio un error durante el manejo de la respuesta AJAX.\r\n\r\nLa respuesta de error aparecera en un nuevo popup.");
									var objErrorWindow = window.open('about:blank', 'qcodo_error','menubar=no,toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes,width=1000,height=700,left=50,top=50');
									if(!objErrorWindow)
                                        return;
                                    objErrorWindow.focus();
									...
									ajax.send('msg="'+msgError+'"');*/
									// ahora alerto
									alert("La respuesta del servidor no se pudo interpretar correctamente.\r\n\r\nEs probable que no se haya efectuado la accion requerida.");
									location.reload();
								}
								//return;
							};
						...
						if (qcodo.objAjaxWaitIcon) {
                            ...
                            if (qcodo.objLoadingPanel) {
                                qcodo.objLoadingPanel.style.display = 'none';
                            }
                        }
