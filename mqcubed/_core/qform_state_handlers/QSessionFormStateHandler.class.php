...
class QSessionFormStateHandler extends QBaseClass {
    ...
    protected static $intStateIndex = null;
    ...
    public static function Save($strFormState, $blnBackButtonFlag) {
        ...
        if (!array_key_exists('qform_current_state_index'.self::$prefix, $_SESSION))
            $_SESSION['qform_current_state_index'.self::$prefix] = 0;
        ...
        if ($blnBackButtonFlag) {
            $_SESSION['qform_current_state_index'.self::$prefix] = $_SESSION['qform_current_state_index'.self::$prefix] + 1;
            $intStateIndex = $_SESSION['qform_current_state_index'.self::$prefix];
        } else {
            $intStateIndex = self::$intStateIndex;
        }
        ...
        $_SESSION['qform_' .self::$prefix . $intStateIndex] = $strFormState;
    ...
    public static function Load($strPostDataState) {
        ...
        self::$intStateIndex = $intStateIndex;
        ...
        if (array_key_exists('qform_'.self::$prefix.$intStateIndex, $_SESSION)) {
            $strSerializedForm = $_SESSION['qform_'.self::$prefix.$intStateIndex];
...
QSessionFormStateHandler::$prefix = defined('__APP_SHORT_NAME__') ? '_'.__APP_SHORT_NAME__ : '';
