...
if (!extension_loaded("pdo") || !extension_loaded("pdo_pgsql")) {
    throw new QPostgreSqlPdoDatabaseException("Unable to load PostgreSql PDO driver");
}
...
class QPostgreSqlPdoDatabase extends QPdoDatabase {
    ...
    public function Connect() {
        ...
        $strDsn = sprintf("%s:%s %s%s", QPostgreSqlPdoDatabase::PDO_PGSQL_DSN_IDENTIFIER, $host, $dbname, $port);
        ...
        try {
            ...
            //FIXME: IMPLEMENTAR CATCHEO DE OTRO ERROR, TIRANDO EL MENSAJE PARA ARRIBA
            throw new QPostgreSqlPdoDatabaseException(sprintf("Unable to connect to Database: %s", $expPgSql->getMessage()), -1, null);
        }

        //$this->objPdo->exec("SET CLIENT_ENCODING TO 'UTF8'");
        // Update Connected Flag
        $this->blnConnectedFlag = true;
        if(isset($this->objConfigArray['schema']) && count($this->objConfigArray['schema'])) {
            $this->NonQuery(sprintf("set search_path to %s;",  $this->strCommaSeparatedSchema));
        }
    }

    public function NonQuery($strNonQuery, $objPreparedStatement=null, $mixParameterArray=null) {
        try {
            return parent::NonQuery($strNonQuery, $objPreparedStatement, $mixParameterArray);
        } catch (QPdoDatabaseException $e) {
            throw new QPostgreSqlPdoDatabaseException($e->getMessage(), $e->ErrorNumber, $e->Query);
        }
    }
    ...
    public function SqlVariable($mixData, $blnIncludeEquality = false, $blnReverseEquality = false) {
        ...
        return $strToReturn . sprintf("E'%s'", addslashes($mixData));
    }
    ...
    public function SqlLimitVariableSuffix($strLimitInfo) {
        ...
        if (strlen($strLimitInfo)) {
            ...
            if (count($strArray) == 2) {
                ...
                throw new QPostgreSqlPdoDatabaseException('Invalid Limit Info: ' . $strLimitInfo, 0, null);
            }
        }
    ...
    public function GetTables() {
        $objResult = $this->Query(sprintf("SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA in (%s) ORDER BY TABLE_NAME ASC", $this->strCommaSeparatedSchema));
        $strToReturn = array();
    ...
    public function GetFieldsForTable($strTableName) {
        ...
        $strQuery = sprintf("
			...
			", $this->strCommaSeparatedSchema, $strTableName);
    ...
    public function GetForeignKeysForTable($strTableName) {
        ...
        $strQuery = sprintf("
				...
				AND
					pc.contype = 'f'
			", $this->SqlVariable($strTableName),$this->strCommaSeparatedSchema);
    ...
    public function Query($strQuery) {
        ...
        if ($objResult === false) {
            ...
            throw new QPostgreSqlPdoDatabaseException($arrPdoError[2], $this->objPdo->errorCode(), $strQuery);
        }
	...
	public function Version() {
		...
		$row = $stmt->GetNextRow();
		$version = explode(' ',$row->GetColumn("version"));
		return str_replace(",","",$version[1]);
	}
        
        public function __sleep() {
            return array('objConfigArray');
        }
    ...
    public function GetColumn($strColumnName, $strColumnType = null) {
        if (array_key_exists($strColumnName, $this->strColumnArray)) {
            ...
            switch ($strColumnType) {
                ...
                case QDatabaseFieldType::Smallint:
...
class QPostgreSqlPdoDatabaseField extends QDatabaseFieldBase {
    ...
    public function __construct($mixFieldData, $objDb = null) {
        ...
        $objResult = $objDb->Query(sprintf("
			...
			", $objDb->SqlVariable($this->strTable),$objDb->strCommaSeparatedSchema));
        ...
        switch ($this->strType) {
            case 'integer':
                ...
                break;
            case 'smallint':
                $this->strType = QDatabaseFieldType::Smallint;
                break;
            case 'money':
                // NOTE: The money type is deprecated in PostgreSQL.
                throw new QPostgreSqlPdoDatabaseException('Unsupported Field Type: money.  Use numeric or decimal instead.', 0, null);
                break;
            case 'bigint':
            case 'decimal':
            case 'numeric':
            ...
            case 'bit':
                if ($this->intMaxLength == 1)
                    ...
                    throw new QPostgreSqlPdoDatabaseException('Unsupported Field Type: bit with MaxLength > 1', 0, null);
                break;
            case 'boolean':
                $this->strType = QDatabaseFieldType::Bit;
                break;
            ...
            case 'time without time zone':
                ...
                break;
            default:
                throw new QPostgreSqlPdoDatabaseException('Unsupported Field Type: ' . $this->strType, 0, null);
        }
    }
