            ...
            foreach (QCodeGen::$SettingsXml->dataSources->children() as $objChildNode) {
                switch (dom_import_simplexml($objChildNode)->nodeName) {
                    case 'database':
                        ...
                        break;
                    case 'oltp_database':
                        QCodeGen::$CodeGenArray[] = new QOLTPCodeGen($objChildNode);
                        break;
                    case 'restService':
...
//			array_push($strToReturn, QRestServiceCodeGen::GenerateAggregateHelper($objRestServiceCodeGen));
    ...
    public function GenerateFiles($strTemplatePrefix, $mixArgumentArray) {
        ...
        if (is_dir($strTemplatePathCustom)) {
            ...
            while ($strModuleName = readdir($objDirectory))
                if ((!in_array($strModuleName, self::$strDirectoriesToExcludeArray)) && is_dir($strTemplatePathCustom . '/' . $strModuleName)) {
                    $objModuleDirectory = opendir($strTemplatePathCustom . '/' . $strModuleName);
                    while ($strFilename = readdir($objModuleDirectory)) {
                        if ((QString::FirstCharacter($strFilename) == '_') && (substr($strFilename, strlen($strFilename) - 4) == '.tpl'))
        ...
        if (is_dir($strTemplatePathCustom)) {
            ...
            while ($strModuleName = readdir($objDirectory)) {
                if ((!in_array($strModuleName, self::$strDirectoriesToExcludeArray)) && is_dir($strTemplatePathCustom . '/' . $strModuleName)) {
                    ...
                    while ($strFilename = readdir($objModuleDirectory)) {
                        if ((QString::FirstCharacter($strFilename) == '_') && (substr($strFilename, strlen($strFilename) - 4) == '.tpl'))
                            if (!isset($strTemplateArray[$strModuleName][$strFilename])) {
                                error_log("$strModuleName, $strFilename");
                                if (!$this->GenerateFile($strTemplatePrefix . '/' . $strModuleName, $strFilename, true, $mixArgumentArray))
                                    $blnSuccess = false;
                            }
                    }
                }
    ...
    protected function EvaluateSubTemplate($strSubTemplateFilename, $strModuleName, $mixArgumentArray) {
        if (QCodeGen::DebugMode)
            _p("Evaluating $strSubTemplateFilename<br/>", false);
    ...
    protected function ClassNameFromTableName($strTableName) {
        ...
        return sprintf('%s%s%s', $this->strClassPrefix, QConvertNotation::CamelCaseFromUnderscore($strTableName), $this->strClassSuffix);
    }
    ...
    protected function ParameterCleanupFromColumn(QColumn $objColumn, $blnIncludeEquality = false) {
        if ($blnIncludeEquality)
            return sprintf('$%s = $objDatabase->SqlVariable($%s, true);', $objColumn->VariableName, $objColumn->VariableName);
        else
            return sprintf('$%s = $objDatabase->SqlVariable($%s);', $objColumn->VariableName, $objColumn->VariableName);
    }
    ...
    protected function CalculateObjectPropertyName($strTableName, $strColumnName, $strReferencedTableName) {
        return sprintf('%s%s%s', $this->strAssociatedObjectPrefix, $this->CalculateObjectDescription($strTableName, $strColumnName, $strReferencedTableName, false), $this->strAssociatedObjectSuffix);
    }
    ...
    protected function CalculateObjectDescription($strTableName, $strColumnName, $strReferencedTableName, $blnPluralize) {
        ...
        if ($strTableName == $strReferencedTableName) {
            ...
            return sprintf("%sAs%s", $strToReturn, $strColumnName);
        } else {
            // If Column Name is only the name of the referenced table, or the name of the referenced table with "_id",
            // then the object description is simply based off the table name.
            if (($strColumnName == $strReferencedTableName) ||
            ...
            return sprintf("%sAs%s", $strToReturn, QConvertNotation::CamelCaseFromUnderscore($strColumnName));
        }
    }
