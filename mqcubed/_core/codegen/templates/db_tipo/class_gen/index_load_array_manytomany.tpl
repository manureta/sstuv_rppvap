...
<% if ( array_key_exists('orden', $objManyToManyReference->ColumnArray ) ) { %>					QQ::Clause($objOptionalClauses,QQ::OrderBy(QQN::<%= $objTable->ClassName %>()-><%= $objManyToManyReference->ObjectDescription %>->Orden)) <% } %><% if ( !array_key_exists('orden', $objManyToManyReference->ColumnArray ) ) { %>					$objOptionalClauses <% } %><%-%>
				);
