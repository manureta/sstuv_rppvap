...
<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
				case '<%=$objReference->ObjectDescription%>':
					 ...
					 */
					return $this->obj<%=$objReference->ObjectDescription%>;
				...
				case '<%=$objReference->ObjectDescription%>Array':
					 ...
					 * @return <%= $objReference->VariableType %>[]
					 */
                                        if(count($this->obj<%=$objReference->ObjectDescription%>Array)==0)
                                             $this->obj<%=$objReference->ObjectDescription%>Array = $this->Get<%= $objReference->ObjectDescription %>Array();
					return (array) $this->obj<%=$objReference->ObjectDescription%>Array;
