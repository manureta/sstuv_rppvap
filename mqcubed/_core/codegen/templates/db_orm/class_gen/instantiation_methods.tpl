...
%><% if ($intCount && (count($objTable->PrimaryKeyColumnArray) == 1)) { %>
		...
		<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
						...
						if ((array_key_exists($strAlias, $strExpandAsArrayNodes)) &&
							...
							if ($intPreviousChildItemCount = count($objPreviousItem->obj<%= $objReference->ObjectDescription %>Array)) {
								$objPreviousChildItems = $objPreviousItem->obj<%= $objReference->ObjectDescription %>Array;
								...
								if ($objChildItem) {
									$objPreviousItem->obj<%= $objReference->ObjectDescription %>Array[] = $objChildItem;
								}
							} else {
								$objPreviousItem->obj<%= $objReference->ObjectDescription %>Array[] = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__<%= $objReference->OppositeColumn %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
							}
		...
		<% foreach ($objTable->ReverseReferenceArray as $objReference) { %><% if (!$objReference->Unique) { %>
						...
						if ((array_key_exists($strAlias, $strExpandAsArrayNodes)) &&
							...
							if ($intPreviousChildItemCount = count($objPreviousItem->obj<%= $objReference->ObjectDescription %>Array)) {
								...
								$objPreviousItem->obj<%= $objReference->ObjectDescription %>Array[] = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
							}
							$blnExpandedViaArray = true;
						}
...
<% foreach ($objTable->PrimaryKeyColumnArray as $col) { %>
	...
	<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
					if (array_diff($objPreviousItem->obj<%= $objReference->ObjectDescription %>Array, $objToReturn->obj<%= $objReference->ObjectDescription %>Array) != null) {
						continue;
					}
	<% } %>
...
<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
			...
			if (!is_null($objDbRow->GetColumn($strAliasName))) {
				if (($strExpandAsArrayNodes) && (array_key_exists($strAlias, $strExpandAsArrayNodes)))
					$objToReturn->obj<%= $objReference->ObjectDescription %>Array[] = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__<%= $objReference->OppositeColumn %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
				else
					$objToReturn->obj<%= $objReference->ObjectDescription %> = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__<%= $objReference->OppositeColumn %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
			}
...
<% foreach ($objTable->ReverseReferenceArray as $objReference) { %><% if (!$objReference->Unique) { %>
			...
			if (!is_null($objDbRow->GetColumn($strAliasName))) {
				if (($strExpandAsArrayNodes) && (array_key_exists($strAlias, $strExpandAsArrayNodes)))
					$objToReturn->obj<%= $objReference->ObjectDescription %>Array[] = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
				else
					$objToReturn->obj<%= $objReference->ObjectDescription %> = <%= $objReference->VariableType %>::InstantiateDbRow($objDbRow, $strAliasPrefix . '<%= strtolower($objReference->ObjectDescription) %>__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
			}
