...
<% foreach ($objTable->ColumnArray as $objColumn) { %>
	<% if ($objColumn->Reference && (!$objColumn->Reference->IsType)) { %>
				$strComplexTypeArray = <%= $objColumn->Reference->VariableType%>::AlterSoapComplexTypeArray($strComplexTypeArray);
	<% } %>
<% } %>
			}
...
<% if ($objColumn->VariableType != QType::DateTime) { %>
    ...
    <% if ((!$objColumn->Identity) && $objColumn->PrimaryKey) { %>
                $objToReturn->__<%= $objColumn->VariableName %> = $objSoapObject-><%= $objColumn->PropertyName %>;
    <% } %>
<% } %><% if ($objColumn->VariableType == QType::DateTime) { %>
