...
<% foreach ($objTable->ManyToManyReferenceArray as $objReference) { %>
	 * @property-read <%= $objReference->VariableType %> $<%=$objReference->ObjectDescription%> the value for the private _obj<%=$objReference->ObjectDescription%> (Read-Only) if set due to an expansion on the <%=$objReference->Table%> association table
	 * @property-read <%= $objReference->VariableType %>[] $<%=$objReference->ObjectDescription%>Array the value for the private _obj<%=$objReference->ObjectDescription%>Array (Read-Only) if set due to an ExpandAsArray on the <%=$objReference->Table%> association table
<% } %><% foreach ($objTable->ReverseReferenceArray as $objReference) { %><% if (!$objReference->Unique) { %>
	 * @property-read <%= $objReference->VariableType %> $<%=$objReference->ObjectDescription%> the value for the private _obj<%=$objReference->ObjectDescription%> (Read-Only) if set due to an expansion on the <%=$objReference->Table%>.<%=$objReference->Column%> reverse relationship
	 * @property-read <%= $objReference->VariableType %>[] $<%=$objReference->ObjectDescription%>Array the value for the private _obj<%=$objReference->ObjectDescription%>Array (Read-Only) if set due to an ExpandAsArray on the <%=$objReference->Table%>.<%=$objReference->Column%> reverse relationship
<% } %><% } %>
