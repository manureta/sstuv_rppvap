...
<% foreach ($objTypeTable->NameArray as $intKey=>$strValue) { %>
				case <%= $intKey %>: return <% if (in_array(QCodeGen::LookupSetting(QCodeGen::$SettingsXml, 'extraColumnParameter', 'column'), $objTypeTable->ExtraFieldNamesArray)) { %><%= $objTypeTable->ClassName %>::To<%= QCodeGen::LookupSetting(QCodeGen::$SettingsXml, 'extraColumnParameter', 'column') %>($int<%= $objTypeTable->ClassName %>Id) . ' - ' . <% } %> '<%= $strValue %>';
<% } %>
				default:
