...
class QDatabaseCodeGen extends QCodeGen {
    ...
    protected $strTipoTableSuffix;
    protected $intTipoTableSuffixLength;
    ...
    public function GetTable($strTableName) {
        $aux = explode(".", $strTableName);
        if(count($aux)==2)$strTableName = $aux[1];
        ...
        else if (array_key_exists($strTableName, $this->objTipoTableArray))
            return $this->objTipoTableArray[$strTableName];
    ...
    public function ValidateTableColumn($strTableName, $strColumnName) {
        ...
        if (array_key_exists($strTableName, $this->objTableArray))
            ...
            $strTableName = $this->objTipoTableArray[$strTableName];
        else
            return false;
    ...
    public function GetReportLabel() {
        // Setup Report Label
        $intTotalTableCount = count($this->objTableArray) + count($this->objTypeTableArray) + count($this->objTipoTableArray);
    ...
    public function GenerateAll() {
        ...
        if (QCodeGenBase::$OnlyTable && $this->objTableArray)
            foreach ($this->objTableArray as $objTable) {
                if ($objTable->Name == QCodeGenBase::$OnlyTable)
        ...
        if (QCodeGenBase::$OnlyTable && $this->objTypeTableArray)
            foreach ($this->objTypeTableArray as $objTypeTable) {
                if ($objTypeTable->Name == QCodeGenBase::$OnlyTable)
        ...
        else if ($this->objTypeTableArray) {
                foreach ($this->objTypeTableArray as $objTypeTable) {
                    if ($this->GenerateTypeTable($objTypeTable))
                        $strReport .= sprintf("Successfully generated DB Type Class:  %s\n", $objTypeTable->ClassName);
                    else
        ...
        // Iterate through all the TIPO tables, generating one class at a time
        if (QCodeGenBase::$OnlyTable && $this->objTipoTableArray)
            foreach ($this->objTipoTableArray as $objTable) {
                if ($objTable->Name == QCodeGenBase::$OnlyTable)
                    if ($this->GenerateTipoTable($objTable)) {
                        $intCount = $objTable->ReferenceCount;
                        if ($intCount == 0)
                            $strCount = '(with no relationships)';
                        else if ($intCount == 1)
                            ...
                            $strCount = sprintf('(with %s relationships)', $intCount);
                        $strReport .= sprintf("Successfully generated DB Tipo Class:   %s %s\r\n", $objTable->ClassName, $strCount);
                    } else
                        $strReport .= sprintf("FAILED to generate DB Tipo Class:       %s\r\n", $objTable->ClassName);
            }
        else if ($this->objTipoTableArray)
            foreach ($this->objTipoTableArray as $objTable) {
                if ($this->GenerateTipoTable($objTable)) {
                    $intCount = $objTable->ReferenceCount;
                    if ($intCount == 0)
...
/*            $objTableArray = array();
            foreach ($objCodeGenArray as $objCodeGen) {
                $objCurrentTableArray = $objCodeGen->TipoTableArray;
                if ($objCurrentTableArray) foreach ($objCurrentTableArray as $objTable)
                    $objTableArray[$objTable->ClassName] = $objTable;
            }

            $mixArgumentArray = array('objTableArray' => $objTableArray);
            if ($objCodeGenArray[0]->GenerateFiles('aggregate_db_tipo', $mixArgumentArray))
                $strToReturn[] = 'Successfully generated Aggregate DB Tipo file(s)';
            else
                $strToReturn[] = 'FAILED to generate Aggregate DB Tipo file(s)';
 * 
 */
        }
    ...
    public function __construct($objSettingsXml) {
        ...
        // Capturamos el error, ya que puede no estar implementado en algun proyecto
        try {
            $this->strTipoTableSuffix = QCodeGen::LookupSetting($objSettingsXml, 'tipoTableIdentifier', 'suffix');
            $this->intTipoTableSuffixLength = strlen($this->strTypeTableSuffix);
        } catch (Exception $e) {
            //
        }
...
//					_p("ASSN Table: $strTableName<br />", false);
                } else if (($this->intTipoTableSuffixLength) &&
                        (strlen($strTableName) > $this->intTipoTableSuffixLength) &&
                        (substr($strTableName, strlen($strTableName) - $this->intTipoTableSuffixLength) == $this->strTipoTableSuffix)) {
                    // Create a TYPE Table and add it to the array
                    $objTipoTable = new QTable($strTableName);
                    $this->objTipoTableArray[strtolower($strTableName)] = $objTipoTable;
//					_p("TIPO Table: $strTableName<br />", false);
                } else {
...
//					_p("Table: $strTableName<br />", false);
        ...
        // Analyze All the Tipo Tables
        if ($this->objTipoTableArray)
            foreach ($this->objTipoTableArray as $objTable)
                $this->AnalyzeTipoTable($objTable);
...
//						$objReferencedTable = $this->objTableArray[strtolower($objReference->Table)];
    ...
    public function GenerateTipoTable(QTable $objTable) {
        // Create Argument Array
        $mixArgumentArray = array('objTable' => $objTable);
        return $this->GenerateFiles('db_tipo', $mixArgumentArray);
    }
    ...
    protected function AnalyzeAssociationTable($strTableName) {
        ...
        // Association tables must have 2 fields DEPRECATED
        // Association tables must have 2 fields or more

        if (count($objFieldArray) < 2) {
            $this->strErrors .= sprintf("AssociationTable %s does not have 2 fields or more.\n",
                            $strTableName);
            return;
        }
        ...
        if (count($objForeignKeyArray) < 2) {
            $this->strErrors .= sprintf("AssociationTable %s does not have  2 foreign keys or more.  Code Gen analysis found %s.\n",
        ...
        for ($intIndex = 0; $intIndex < 2; $intIndex++) {
            ...
            $objManyToManyReference->AssociatedTable = substr( $objOppositeForeignKey->ReferenceTableName, strpos($objOppositeForeignKey->ReferenceTableName, "."));
                ...
                if (count($objForeignKey->ColumnNameArray) != 1)
                    ...
                    if (array_key_exists(strtolower($strColumnName), $objTable->ColumnArray) &&
                        ...
                        if ((array_key_exists(strtolower($objForeignKey->ReferenceTableName), $this->objTableArray)) ||
                            ...
                            (array_key_exists(strtolower($objForeignKey->ReferenceTableName), $this->objTipoTableArray))) {
...
//								$objReferencedTable = $this->objTableArray[strtolower($objReference->Table)];
    ...
    protected function AnalyzeTipoTable(QTable $objTable) {
        // Setup the Table Object
        $strTableName = $objTable->Name;
        $objTable->ClassName = $this->ClassNameFromTableName($strTableName);
        $objTable->ClassNamePlural = $this->Pluralize($objTable->ClassName);
...
//								$objReferencedTable = $this->objTableArray[strtolower($objReference->Table)];
        ...
        if (count($strMatches) && ($strMatches[0] == $strTableName) && ($strTableName != '_')) {
            ...
            foreach ($strReservedWords as $strReservedWord)
                if ($strTableNameToTest == $strReservedWord) {
                    $this->strErrors .= sprintf("Table '%s' has a table name which is a PHP reserved word.\r\n", $strTableName);
                    unset($this->objTipoTableArray[strtolower($strTableName)]);
                    return;
                }
        } else {
            $this->strErrors .= sprintf("Table '%s' can only contain characters that are alphanumeric or _, and must not begin with a number.\r\n", $strTableName);
    ...
    public function __get($strName) {
        switch ($strName) {
            case 'TableArray':
                return array_merge($this->objTableArray, $this->objTipoTableArray);
            case 'OrmTableArray':
                return $this->objTableArray;
            case 'TipoTableArray':
                return $this->objTipoTableArray;
            case 'TypeTableArray':
