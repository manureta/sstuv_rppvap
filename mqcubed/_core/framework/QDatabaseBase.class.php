<?php
			...
			switch ($strName) {
				...
				case 'BackupFile':
					return $this->objConfigArray[strtolower($strName)];
                                case 'strSchemaArray':
                                	return array_map("trim",$this->objConfigArray['schema']);
                                case 'strCommaSeparatedSchema':
                                	return (isset($this->objConfigArray['schema']) && count($this->objConfigArray['schema']))?"'".implode("','", array_map("trim",$this->objConfigArray['schema']))."'":'current_schema()';
				default:
					try {
	...
	abstract class QDatabaseNumberFieldMax {
		const Smallint = 32767;
		const Integer = 2147483647;
		#const Bigint = 9223372036854775807;
	}

	abstract class QDatabaseNumberFieldMin {
		const Smallint = -32768;
