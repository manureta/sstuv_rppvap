...
if (!defined('__DATAGRID_DATE_FORMAT__')) {
    define('__DATAGRID_DATE_FORMAT__', "DD/MM/YYYY");
}
...
class QQNode extends QQBaseNode {
    ...
    public function SetFilteredDataGridColumnFilter(QFilteredDataGridColumn $col) {
        ...
        if (isset($this->strClassName)) {
            $strClass = $this->strClassName;
            $strPk = $this->_PrimaryKeyNode->_PropertyName;
            $col->SetNode($this);
            return;
        }
        ...
        switch ($this->strType) {
            ...
            case QType::DateTime:
                ...
                if ($this->strType == QType::DateTime)
                    $col->FilterType = QFilterType::DateFilter;
                $col->FilterPrefix = '%';
                $col->FilterPostfix = '%';
            ...
            case QType::Float: case QType::Integer:
                //EQUAL
                $col->FilterType = QFilterType::NumericTextFilter;
...
//				}
    ...
    public function GetDataGridHtml() {
        ...
        // Simple Two-Step Node
            ...
            if($objNodeArray[1]->strType == QType::DateTime) {
                $strToReturn = sprintf('(%s) ? %s->qFormat("%s"):""', 
                        $strToReturn, $strToReturn, __DATAGRID_DATE_FORMAT__);
            }
        }           
        ...
        // Complex N-Step Node
            ...
            for ($intIndex = 2; $intIndex < count($objNodeArray); $intIndex++) {
                ...
                if($objNodeArray[$intIndex]->strType == QType::DateTime) {
                    $strNodeLabelArray[$intIndex - 1] = sprintf('(%s) ? %s->qFormat("%s"):""', 
                        $strNodeLabelArray[$intIndex-1], $strNodeLabelArray[$intIndex-1], __DATAGRID_DATE_FORMAT__);
                }
            }
        ...
        return '(string) ' . $strToReturn; //se rompía con nulls en nodos dependientes
    }
...
class QQAssociationNode extends QQBaseNode {
    ...
    public function GetColumnAlias(QQueryBuilder $objBuilder, $blnExpandSelection = false, QQCondition $objJoinCondition = null) {
        ...
        // If we are a standard QQNode at the top level column, simply return the column name
        if ((get_class($this) == 'QQNode') && (is_null($this->objParentNode->_Type)))
            return sprintf('%s%s%s.%s%s%s', $strBegin, $this->objParentNode->_Name, $strEnd, $strBegin, $this->strName, $strEnd);
        else {
            // Use the Helper to Iterate Through the Parent Chain and get the Parent Alias
            $strParentAlias = $this->objParentNode->GetColumnAliasHelper($objBuilder, $strBegin, $strEnd, $blnExpandSelection);
            ...
            if ($this->strTableName) {
                // Next, Join the Appropriate Table
                $objBuilder->AddJoinItem($this->strTableName, $strParentAlias . '__' . $this->strName, $strParentAlias, $this->strName, $this->strPrimaryKey);
            ...
            // Finally, return the final column alias name (Parent Prefix with Current Node Name)
            return sprintf('%s%s%s.%s%s%s', $strBegin, $strParentAlias, $strEnd, $strBegin, $this->strName, $strEnd);
        }
    }
    ...
    public function GetColumnAliasHelper(QQueryBuilder $objBuilder, $strBegin, $strEnd, $blnExpandSelection) {
        ...
        // Yep -- Simply return the Parent Node Name
            ...
            // Next, Join the Appropriate Table
            $objBuilder->AddJoinItem($this->strTableName, $strParentAlias . '__' . $this->strName, $strParentAlias, $this->objParentNode->_PrimaryKey, $this->strPrimaryKey);
...
class QQConditionLike extends QQConditionComparison {
    ...
    public function UpdateQueryBuilder(QQueryBuilder $objBuilder) {
        if ($objBuilder->Database instanceof QPostgreSqlDatabase || $objBuilder->Database instanceof QPostgreSqlPdoDatabase) {
            if ($this->mixOperand instanceof QQNamedValue)
...
class QQConditionNotLike extends QQConditionComparison {
    ...
    public function UpdateQueryBuilder(QQueryBuilder $objBuilder) {
        if ($objBuilder->Database instanceof QPostgreSqlDatabase || $objBuilder->Database instanceof QPostgreSqlPdoDatabase) {
            if ($this->mixOperand instanceof QQNamedValue)
                $objBuilder->AddWhereItem($this->objQueryNode->GetColumnAlias($objBuilder) . '::text NOT ILIKE ' . $this->mixOperand->Parameter());
            else
                $objBuilder->AddWhereItem($this->objQueryNode->GetColumnAlias($objBuilder) . '::text NOT ILIKE ' . $objBuilder->Database->SqlVariable($this->mixOperand));
        }
        else {
            if ($this->mixOperand instanceof QQNamedValue)
                $objBuilder->AddWhereItem($this->objQueryNode->GetColumnAlias($objBuilder) . ' NOT ILIKE ' . $this->mixOperand->Parameter());
            else
                $objBuilder->AddWhereItem($this->objQueryNode->GetColumnAlias($objBuilder) . ' NOT ILIKE ' . $objBuilder->Database->SqlVariable($this->mixOperand));
        }
    }
...
class QQueryBuilder extends QBaseClass {
    ...
    public function AddSelectFunction($strFunctionName, $strColumnName, $strFullAlias) {
        $this->strSelectArray[$strFullAlias] = sprintf('%s(%s) AS %s__%s%s', $strFunctionName, $strColumnName, $this->strEscapeIdentifierBegin, $strFullAlias, $this->strEscapeIdentifierEnd);
    }
    ...
    public function GetVirtualNode($strName) {
        ...
        if (array_key_exists($strName, $this->objVirtualNodeArray))
            ...
            throw new QCallerException('Undefined Virtual Node: ' . $strName);
    }
