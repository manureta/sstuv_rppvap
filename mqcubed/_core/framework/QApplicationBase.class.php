...
abstract class QApplicationBase extends QBaseClass {
    ...
    public static function InitializeDatabaseConnections() {
        for ($intIndex = 0; $intIndex <= 100; $intIndex++) {
            ...
            if (defined($strConstantName)) {
                ...
                $strExpectedKeys = array(
                        ...
                        'username', 'password', 'profiling', 'schema'
                );
                ...
                if (!class_exists($strDatabaseType)) {
                    ...
                    require_once($strDatabaseAdapter);
                }

                QApplication::$Database[$intIndex] = new $strDatabaseType($intIndex, $objConfigArray);
                if(is_array($objConfigArray) && isset($objConfigArray['schema']) && is_array($objConfigArray['schema']) && count($objConfigArray['schema']) > 0 && isset($objConfigArray['database']) && $objConfigArray['database'] != '') {
                    $objDb = QApplication::$Database[$intIndex];
                    $objDb->NonQuery(sprintf("set search_path to %s;",  implode(",", $objConfigArray['schema'])));
                }
            }
        }
    }
    ...
    public static function Redirect($strLocation) {
        ...
        if ((QApplication::$RequestMode == QRequestMode::Ajax) ||
            ...
            // Hide the body to prevent user to commit another action
            $strLocation = 'qc.redirect("' . $strLocation . '");';
            ...
            if (array_key_exists('DOCUMENT_ROOT', $_SERVER) && ($_SERVER['DOCUMENT_ROOT'])) {
                ...
                printf('<script type="text/javascript">location.href = "%s";</script>', $strLocation);
            }
        }
    ...
    public static $NotificationMessageArray = array();
    ...
    public static function DisplayNotification($strMessage, $chrType='S') {
        array_push(QApplication::$NotificationMessageArray, array($strMessage, $chrType));
    }
    ...
    public static function RenderJavaScript($blnOutput = true) {
        ...
        foreach (QApplication::$AlertMessageArray as $strAlert) {
            ...
            $strAlert = str_replace("\n\r", '\n', $strAlert);
            $strAlert = str_replace("\r\n", '\n', $strAlert);
            $strAlert = str_replace("\r", '\n', $strAlert);
            $strAlert = str_replace("\n", '\n', $strAlert);
            $strScript .= sprintf('bootbox.alert("%s"); ', $strAlert);
        }
        foreach (QApplication::$NotificationMessageArray as $arrNotify) {
            $strNotify = nl2br($arrNotify[0]);
            $strNotify = addslashes($strNotify);
            $strNotify = str_replace("\n\r", '\n', $strNotify);
            $strNotify = str_replace("\r\n", '\n', $strNotify);
            $strNotify = str_replace("\r", '\n', $strNotify);
            $strNotify = str_replace("\n", '\n', $strNotify);
            switch(strtolower($arrNotify[1])){
                case 'error':
                case 'e':   $strClass = 'gritter-error'; break;
                case 'success':
                case 's':   $strClass = 'gritter-success'; break;
                case 'warning':
                case 'w':   $strClass = 'gritter-warning'; break;
                case 'info':
                case 'i':   $strClass = 'gritter-info'; break;
                case 'light':
                case 'l':   $strClass = 'gritter-light'; break;
                default:    $strClass = ''; break;
            }
            $strScript .= sprintf('$.gritter.add({text: "%s",class_name: "%s",time:3000});', $strNotify, $strClass);
        }
        ...
        QApplication::$NotificationMessageArray = array();
