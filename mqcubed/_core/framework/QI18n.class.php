		...
		public function TranslateToken($strToken) {
			...
			if (array_key_exists($strCleanToken, $this->strTranslationArray))
				return $this->strTranslationArray[$strCleanToken];
                        //little hack para los CAlgoObject => Algo
                        if (substr($strToken, -6)==='Object') {
                            if (substr($strToken, 0,1)==='C')
                                return implode(' ',preg_split('/(?=[A-Z])/',(substr($strToken, 1, strpos($strToken, 'Object')-1))));
                            if (substr($strToken, 0,2)==='Id') 
                                return implode(' ',preg_split('/(?=[A-Z])/',(substr($strToken, 2, strpos($strToken, 'Object')-2))));
                        }
                        return $strToken;				
		}
