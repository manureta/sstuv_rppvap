...
//			$this->objColumnArray[count($this->objColumnArray)] = $objColumn;
		...
		public function RemoveColumnByNode(QQNode $objNode) {
			$this->blnModified = true;
			for ($intIndex = 0; $intIndex < count($this->objColumnArray); $intIndex++)
				if ($this->objColumnArray[$intIndex]->Name == $objNode->_PropertyName) {
		...
		public function GetColumnByName($strName) {
			if ($this->objColumnArray) foreach ($this->objColumnArray as $objColumn)
                ...
                public function GetColumnByPosition($iPosition) {
			if (isset($this->objColumnArray[$iPosition])) 
                            return $this->objColumnArray[$iPosition];
                        else return NULL;
		}
		...
		protected function GetHeaderSortedHtml(QDataGridColumn $objColumn) {
			...
			if ($this->intSortDirection == 0)
				$strToReturn .= sprintf(' <img src="%s/sort_arrow.png" width="7" height="7" border="0" alt="Sorted" />', __IMAGE_ASSETS__);
			else
				$strToReturn .= sprintf(' <img src="%s/sort_arrow_reverse.png" width="7" height="7" border="0" alt="Reverse Sorted" />', __IMAGE_ASSETS__);
...
//                                substr($strHtml, 0, $objColumn->MaxChars).'...';
                        }
                    }
				} catch (QCallerException $objExc) {
					$objExc->IncrementOffset();
