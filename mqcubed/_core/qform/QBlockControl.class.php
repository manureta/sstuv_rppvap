<?php
	...
	abstract class QBlockControl extends QControl {
		...
		protected $blnAutoRenderChildrenWithName = false;
...
*/
		...
		protected function GetControlHtml() {
			...
			$strToReturn .= sprintf('<%s id="%s" %s%s>%s%s%s</%s>',
				...
				$strTemplateEvaluated,
				($this->blnAutoRenderChildrenWithName) ? $this->RenderChildrenWithName(false) : ( ($this->blnAutoRenderChildren) ? $this->RenderChildren(false) : '' ),
				$this->strTagName);
...
//				$strToReturn .= sprintf('<span id="%s_ctldzmask" style="position:absolute;"><span style="font-size: 1px">&nbsp;</span></span>', $this->strControlId);
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case "AutoRenderChildrenWithName": return $this->blnAutoRenderChildrenWithName;
		...
		public function __set($strName, $mixValue) {
			...
			switch ($strName) {
				...
				case "AutoRenderChildrenWithName":
					try {
						$this->blnAutoRenderChildrenWithName = QType::Cast($mixValue, QType::Boolean);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
