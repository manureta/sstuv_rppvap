...
//                                $objToReturn = unserialize($_SESSION[$objForm->FormId . '_' . $strControlId]);
			if (Session::GetSessionVar($objForm->FormId . '_' . $strControlId)) {
                                $objToReturn = unserialize(Session::GetSessionVar($objForm->FormId . '_' . $strControlId));
		...
		public function Persist() {
            ...
            Session::SetSessionVar($this->objForm->FormId . '_' . $this->strControlId, serialize($objControl));
...
/*		public function GetEndHtml() {
		...
		protected function RenderOutput($strOutput, $blnDisplayOutput, $blnForceAsBlockElement = false) {
			...
			else $strStyle .= ($blnIsBlockElement) ? 'display:block;' : 'display:inline;';
...
//		abstract public function GetJavaScriptAction();
		...
		protected function RenderChildrenWithName($blnDisplayOutput = true) {
			$strToReturn = "";

			foreach ($this->GetChildControls() as $objControl)
				if (!$objControl->Rendered)
					$strToReturn .= $objControl->RenderWithName($blnDisplayOutput);
