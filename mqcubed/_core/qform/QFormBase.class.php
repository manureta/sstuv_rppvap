		...
		public static function Run($strFormId = null, $strAlternateHtmlFile = null) {
			...
			if (array_key_exists('Qform__FormId', $_POST) && ($_POST['Qform__FormId'] == $strFormId) && array_key_exists('Qform__FormState', $_POST)) {
				...
				if (!$objClass){ //throw new QInvalidFormStateException($strFormId);
                                    // ugly hack, if there is no http_referer, go to approot
                                    if (!isset($_SERVER['HTTP_REFERER'])) $_SERVER['HTTP_REFERER'] = __VIRTUAL_DIRECTORY__;
                                    if(isset($_POST['Qform__FormCallType']) && $_POST['Qform__FormCallType']==QCallType::Ajax){
                                                header('Content-Type: text/xml');
                                                //Si la sesión expiró debemos elimarla y también del browser
                                                ...
                                                printf('<' . '?xml version="1.0"?' . '><response><controls/><commands><command>alert("%s");document.location="%s"</command></commands></response>',
                                                    QApplicationBase::Translate("Session has expired. No action was carried out, please try again later."),
			...
			if ($objClass) {
				...
				$$strFormId = $objClass;
                                ...
                                $objClass->strCallType = isset($_POST['Qform__FormCallType'])?$_POST['Qform__FormCallType']:QCallType::None;
		...
		public function RemoveControl($strControlId) {
			if (array_key_exists($strControlId, $this->objControlArray)) {
				...
				if (array_key_exists($strControlId, $this->objPersistentControlArray)) {
                    ...
                    Session::SetSessionVar($this->strFormId . '_' . $strControlId, null);
...
/*				$strControlId = $objParentObject->ControlId;
		...
		public function ValidateControlAndChildren(QControl $objControl) {
                        if (method_exists($objControl, 'ValidateControlAndChildren'))
			...
			if (array_key_exists('Qform__FormControl', $_POST)) {
				...
				if ($strId != '') {
					...
					if (array_key_exists($strId, $this->objControlArray)) {
						...
						throw new QInvalidFormStateException(sprintf('Control passed by Qform__FormControl does not exist: %s', $strId));
				}/* else {
		...
		public function RenderBegin($blnDisplayOutput = true) {
			...
			foreach ($strJavaScriptArray as $strScript) {
                                $strScript = (substr($strScript,0,1)=="/")?$strScript:__JS_ASSETS__."/$strScript";
				$strToReturn .= sprintf('<script type="text/javascript" src="%s"></script>', $strScript);
				$strToReturn .= "\r\n";
			}
			// set server instance value on client
			$strToReturn .= sprintf('<script type="text/javascript" >var SERVER_INSTANCE ="' . SERVER_INSTANCE .'"; PROJECT_NAME = "' . __APP_SHORT_NAME__ .'";</script>', $strScript);
			$strToReturn .= "\r\n";

			// Include styles that need to be included
			foreach ($strStyleSheetArray as $strScript) {
				$strScript = (substr($strScript,0,1)=="/")?$strScript:__CSS_ASSETS__."/$strScript";
				$strToReturn .= sprintf('<style type="text/css" media="all">@import "%s";</style>', $strScript);
				$strToReturn .= "\r\n";
			}
        ...
        public function ResetJavaScriptFiles(){
            $this->strIncludedJavaScriptFileArray = array();
        }
			...
			if ($strJavaScriptFileList = trim($strJavaScriptFileList)) {
				...
				foreach ($strScriptArray as $strScript)
					if ($strScript = trim($strScript)) 
						...
						if (array_search($strScript, $this->strIgnoreJavaScriptFileArray) === false && !array_key_exists($strScript, $this->strIncludedJavaScriptFileArray)) {
		...
		public function RenderEnd($blnDisplayOutput = true) {
            ...
            // BUG si tenes ( A ; B ; C ; D ) la segunda pasada te elimina B ; C ;
/*
...
*/
			...
			$strEndScript = sprintf('qc.jsAssets = "%s"; ', __JS_ASSETS__) . $strEndScript;
			$strEndScript = sprintf('qc.phpAssets = "%s"; ', __PHP_ASSETS__) . $strEndScript;
			$strEndScript = sprintf('qc.cssAssets = "%s"; ', __CSS_ASSETS__) . $strEndScript;
			$strEndScript = sprintf('qc.imageAssets = "%s"; ', __IMAGE_ASSETS__) . $strEndScript;
