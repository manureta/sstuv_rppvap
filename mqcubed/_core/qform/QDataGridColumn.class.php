<?php
	...
	class QDataGridColumn extends QControl {
		// APPEARANCE
		...
		protected $blnCrop = false;
		protected $intMaxChars = 50;
		...
		public function __get($strName) {
			switch ($strName) {
				// APPEARANCE
				case "MaxChars": return $this->intMaxChars;
				case "Crop": return $this->blnCrop;
		...
		public function __set($strName, $mixValue) {
			switch ($strName) {
				...
				case "MaxChars": 
					try {
						$this->intMaxChars = QType::Cast($mixValue, QType::Integer);
						break;
				...
				case "Crop": 
					try {
						$this->blnCrop = QType::Cast($mixValue, QType::Boolean);
						break;
					} catch (QInvalidCastException $objExc) {
						$objExc->IncrementOffset();
						throw $objExc;
