...
abstract class QFilterType {
    ...
    const None = '';
    const NumericTextFilter = 'Numeric';
    const TextFilter = 'Text';
    const ListFilter = 'Advanced List';
    const SimpleListFilter = 'List';
...
class QFilteredDataGridColumn extends QDataGridColumn {
    ...
    public function SetFilterCondition(QQCondition $objCondition) {
        ...
        $this->AddFilterCondition($objCondition);
    }

    public function AddFilterCondition(QQCondition $objCondition) {
    ...
    protected function GetControlHtml() {
        ...
        $strTooltip = ($this->Tooltip) ? sprintf(' title="%s" ', $this->Tooltip) : '';
    ...
    protected function LoadFilterItems() {
        ...
        if($strClass::QueryCount($this->Conditions) >__MAX_RECORD_FILTERBOX__) {
            $this->FilterType = null;
            return false;
        }
        foreach ($strClass::QueryArray($this->Conditions) as $obj) {
            $this->AddListItem((string) $obj, QQ::Equal($this->objNode, $obj->$strPk));
        }
        $this->FilterType = QFilterType::ListFilter;
        return true;
    }
    /////////////////////////
    ...
    public function FilterAddListItem($arg1 = null, $arg2 = null) {
        ...
        elseif ($arg1 !== null) {
            $this->arrFilterList[$arg1] = $arg2;
            $this->strFilterType = QFilterType::SimpleListFilter;
        }
