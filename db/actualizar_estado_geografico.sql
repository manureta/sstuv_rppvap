--select actualizar_estado_nomenclaturas(944)
...
DECLARE
  ...
  _duplicado character varying;
  _es_duplicado boolean;
...
FOR _nomencla IN (SELECT *  FROM nomenclatura WHERE id_folio=_id_folio) LOOP    
    ...
    _es_duplicado:=(select (count(*)>1)::boolean from _parcelas where _parcelas.nomencla=_nom_tmp);
    

    IF _es_duplicado THEN
	    RAISE NOTICE 'Nomencla duplicada';
	    EXECUTE 'UPDATE nomenclatura SET estado_geografico='||quote_literal(_duplicado)||' where id='||_nomencla.id;
	ELSE
	    ...
	    RAISE NOTICE 'Overlapea  %...',_overlaps;
		...
		CASE
		    ...
		    ELSE ' SET estado_geografico='||quote_literal(_desconocido)||' where id='||_nomencla.id 
		 END;
     END IF;	
